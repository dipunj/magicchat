...
#1
    def view_unread(self):
        try:
            ...
            path = os.getcwd()
        script = open(os.path.join(path, "js/get_unread_messages.js"), "r").read()
...
#2
    def send_to_phone_number(self, pno, message):
        ...
        script = open(os.path.join(script_path, "js/send_message_to_phone_number.js"), "r").read()
...
#3
    def get_all_messages(self, id):
        try:
            ...
            script_path = os.getcwd()
        script = open(os.path.join(script_path, "js/get_all_messages.js"), "r").read()
        success = self.driver.execute_script(script, id)
        return success
        
#4 . No of unread messages 
    def get_n_messages(self, id, numberOfMsg):
        try:
            script_path = os.path.dirname(os.path.abspath(__file__))
        except NameError:
            script_path = os.getcwd()
        script = open(os.path.join(script_path, "js/get_n_messages.js"), "r").read()
        success = self.driver.execute_script(script, id, numberOfMsg)
        return success
        
        
#5 Send a message to a whatsapp id
    def send_to_whatsapp_id(self, id, message):
        try:
            script_path = os.path.dirname(os.path.abspath(__file__))
        except NameError:
            script_path = os.getcwd()
        script = open(os.path.join(script_path, "js/send_message_to_whatsapp_id.js"), "r").read()
        success = self.driver.execute_script(script, id.lower(), message)
        return success
#6 get whatsapp id of all chats 
    def get_whatsapp_id(self, pno, message):
        try:
            script_path = os.path.dirname(os.path.abspath(__file__))
        except NameError:
            script_path = os.getcwd()
        script = open(os.path.join(script_path, "js/test.js"), "r").read()
        success = self.driver.execute_script(script, pno, message)
        return success
